...
const createToken = (id: string) =>
  ...
  jwt.sign(
    { id },
    env.JWT_SECRET as unknown as jwt.Secret,
    {
...
export const googleSignIn = async (req: Request, res: Response) => {
  ...
  if (!id_token)
    return res.status(400).json({ error: true, message: 'Missing id_token' });
  ...
  try {
    ...
    if (!email)
      return res
        .status(400)
        .json({ error: true, message: 'Google token missing email' });
    ...
    if (!user) {
      user = await User.create({
        ...
        googlePhotoUrl: info.picture
      });
    } else if (info.picture && user.googlePhotoUrl !== info.picture) {
      user.googlePhotoUrl = info.picture;
