...
const roleFilterOptions: Array<{ value: "all" | "student" | "alumni" | "admin"; label: string }> = [
...
const sortOptions: Array<{ value: "relevance" | "recent" | "alphabetical"; label: string }> = [
...
const ConnectionsPage = () => {
  ...
  const [sortKey, setSortKey] = useState<"relevance" | "recent" | "alphabetical">("relevance");
  ...
  const aggregatedRoleCounts = useMemo(() => {
    ...
    Object.values(tabData).forEach((list) => {
      list.forEach((person) => {
        if (person.role === "student" || person.role === "alumni" || person.role === "admin") {
          tally[person.role] += 1;
        }
      });
    });
  ...
  const totalPeople = aggregatedRoleCounts.student + aggregatedRoleCounts.alumni + aggregatedRoleCounts.admin;
  ...
  const hasActiveFilters = searchTerm.trim().length > 0 || roleFilter !== "all" || sortKey !== "relevance";
  ...
  const renderEmptyState = (title: string, detail?: string) => (
    <div className="neon-border relative overflow-hidden rounded-3xl bg-surface/70 p-10 text-center shadow-[0_28px_60px_-30px_rgba(9,16,33,0.65)]">
      ...
      {detail && <p className="relative mt-2 text-sm text-white/70">{detail}</p>}
      {hasActiveFilters && (
        <button
          type="button"
  ...
  return (
    <section className="space-y-10 pb-12">
        ...
        <div className="relative z-10 flex flex-col gap-6 text-white">
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div className="max-w-xl space-y-3">
              <p className="text-xs uppercase tracking-[0.32em] text-white/70">Network HQ</p>
              <h1 className="text-3xl font-extrabold leading-tight sm:text-4xl">Grow authentic connections</h1>
              <p className="text-sm text-white/70 sm:text-base">
                Manage requests, nurture relationships, and discover collaborators aligned with your goals.
                Your network currently spans <span className="text-white">{totalPeople}</span> community members.
              </p>
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-sm">
                ...
                <p className="text-2xl font-bold text-white">{suggestionCount}</p>
              </div>
            </div>
          </div>
          <div className="flex flex-wrap gap-3 text-xs text-white/70">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1">
              <Users className="h-4 w-4" /> {counts.connections} active connections
            </div>
            <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1">
              <UserPlus className="h-4 w-4" /> {counts.pending} invites awaiting reply
            </div>
            <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1">
              <Sparkles className="h-4 w-4" /> {suggestionCount} new introductions curated for you
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {statHighlights.map(({ id, label, value, helper, icon: Icon, gradient }) => (
          <div
            key={id}
      ...
      <div className="neon-border space-y-5 rounded-3xl bg-surface/70 p-6 shadow-[0_32px_80px_-46px_rgba(7,12,31,0.9)]">
        <div className="grid gap-4 md:grid-cols-[minmax(260px,2fr)_minmax(180px,1fr)]">
          <div className="space-y-2">
            <label htmlFor="connections-search" className="text-xs uppercase tracking-wide text-white/60">
              Search network
            </label>
          ...
          <div className="space-y-2">
            <label htmlFor="connections-sort" className="flex items-center gap-2 text-xs uppercase tracking-wide text-white/60">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {availableRoleFilters.map(({ value, label }) => {
              ...
              const countLabel = value === "all" ? totalPeople : aggregatedRoleCounts[value];
              return (
                <button
                  key={value}
                  type="button"
            ...
            >
              ...
              <span
                className={`inline-flex min-w-[28px] items-center justify-center rounded-full px-2 py-0.5 text-[11px] font-semibold ${
                  isActive ? "bg-surface text-slate-900" : "bg-white/10 text-white/70"
                }`}
              >
                {tabCount}
              </span>
              ...
              const roleLabel =
                person.role === "alumni"
                  ...
                  : person.role === "student"
                    ? "Student"
                    : "Admin";
              const relativeTimestamp = formatRelativeTime(person.requestedAt ?? person.createdAt);
                ...
                activeTab === "pending"
                  ...
                  : activeTab === "outgoing"
                    ...
                    : activeTab === "connections"
                      ? "Connected"
                      : "Spotted";
              const bioCopy = person.bio || "This member hasn’t added a bio yet.";
                ...
                >
                  ...
                  <div className="relative flex items-start gap-4">
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <p className="max-h-14 overflow-hidden text-xs text-white/70">{bioCopy}</p>
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {person.overlap ? (
                          <span className="inline-flex items-center gap-1 rounded-full border border-neonCyan/40 bg-neonCyan/10 px-2 py-1 text-[11px] font-semibold text-neonCyan">
                            <Users className="h-3 w-3" /> {person.overlap} shared skill
                          </span>
                        ) : null}
                      </div>
                      ...
                      {relativeTimestamp && (
                        <p className="flex items-center gap-1 text-[11px] text-white/60">
                          <Clock className="h-3.5 w-3.5" /> {timestampPrefix} {relativeTimestamp}
                        </p>
                      )}
                    </div>
                  </div>
