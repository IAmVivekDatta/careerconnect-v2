...
const AchievementsPage = () => {
  ...
  const { data: badgeSummary, isLoading: isBadgeSummaryLoading } = useQuery<BadgeResponse>({
    queryKey: ["badges", user?._id],
    queryFn: async () => {
      const response = await api.get<BadgeResponse>(`/achievements/${user?._id}/badges`);
      return response.data;
    },
    enabled: Boolean(user?._id)
  });
  ...
  const { data: leaderboardData } = useQuery<LeaderboardEntry[]>({
    ...
    queryFn: async () => {
      const response = await api.get<LeaderboardEntry[]>("/achievements/leaderboard");
      return response.data;
    }
  });

  const { data: questProgress, isLoading: isQuestProgressLoading } = useQuery<QuestProgressResponse>({
    queryKey: ["quest-progress", user?._id],
    queryFn: async () => {
      const response = await api.get<QuestProgressResponse>(`/achievements/${user?._id}/quests`);
      return response.data;
    },
    enabled: Boolean(user?._id)
  });

  const toggleQuestMutation = useMutation<QuestProgressResponse, unknown, string>({
    mutationFn: async (questId) => {
      if (!user?._id) {
        throw new Error("User not ready");
      }
      const response = await api.post<QuestProgressResponse>(`/achievements/${user._id}/quests`, {
        questId
      });
      return response.data;
    },
  ...
  const endorsementCount = useMemo(() => {
    ...
    return Object.values(endorsementMap).reduce((acc, endorsers) => acc + endorsers.length, 0);
  }, [endorsementMap]);

  const highestMomentumSkill = useMemo(() => {
    if (!endorsementMap || Object.keys(endorsementMap).length === 0) return null;
    const sorted = Object.entries(endorsementMap).sort((a, b) => b[1].length - a[1].length);
  ...
  const baseNextMilestone = useMemo(
    () => ACHIEVEMENT_MILESTONES.find((entry) => totalPoints < entry.threshold) ?? null,
    [totalPoints]
  );

  const nextMilestone = useMemo<Milestone>(() => {
    if (baseNextMilestone) return baseNextMilestone;
    const finalMilestone = ACHIEVEMENT_MILESTONES[ACHIEVEMENT_MILESTONES.length - 1];
    return {
      id: "legendary",
  ...
  const milestoneProgress = useMemo(() => {
    const span = Math.max(1, nextMilestone.threshold - previousMilestone.threshold);
    ...
    <section className="space-y-10 pb-12">
      <div className="grid gap-6 xl:grid-cols-[1.75fr,1fr]">
            ...
            <div className="flex flex-wrap items-start justify-between gap-6">
              <div className="max-w-xl space-y-3">
                ...
                <p className="text-sm text-white/70 sm:text-base">
                  Earn experience by sharing insights, endorsing peers, and unlocking opportunities.
                  Keep the streak alive to climb the community leaderboard.
                </p>
              </div>

              <div className="flex flex-col gap-4 rounded-2xl border border-white/15 bg-white/10 p-5 backdrop-blur-md">
                <div className="space-y-2">
                  <p className="text-xs uppercase tracking-wide text-white/70">Lifetime points</p>
            ...
            <div className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-black/20 p-6 backdrop-blur-md sm:flex-row sm:items-center sm:justify-between">
              <div className="space-y-1 max-w-md">
                <p className="text-xs uppercase tracking-wide text-white/60">
                  {milestoneProgress.hasClearedAll ? "Legend status" : "Next milestone"}
                </p>
                <p className="text-lg font-semibold text-white">
              ...
              <div className="flex w-full flex-col gap-2 sm:max-w-md">
                ...
                <p className="text-xs text-white/70">
                  Progress: {milestoneProgress.percent}% toward {milestoneProgress.upcoming.label}
                </p>
              </div>
            </div>
          </div>
        ...
        <aside className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
          {statCards.map(({ id, label, value, helper, icon: Icon, gradient }) => (
            <div
              key={id}
        ...
        <section className="neon-border flex flex-col gap-4 rounded-3xl bg-surface/70 p-6 shadow-[0_32px_70px_-36px_rgba(8,10,26,0.85)]">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h2 className="text-lg font-semibold text-white">Daily power-ups</h2>
            </div>
          ...
          <div className="space-y-4">
            {QUEST_LIBRARY.map((quest) => {
              ...
              const isUpdatingThisQuest =
                toggleQuestMutation.isPending && toggleQuestMutation.variables === quest.id;
              ...
              return (
                <div
                  ...
                  className={`relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-surface/90 via-surface/70 to-surface/60 p-5 transition-all duration-200 ${
                    isCompleted ? "ring-2 ring-emerald-400/40" : "hover:border-cyan-400/50"
                  }`}
                >
