...
const AdminReportsPage = () => {
  ...
  const reportCards: ReportCard[] = useMemo(() => {
    ...
    const engagementRate = ((stats.users.active30d / totalUsers) * 100).toFixed(
      1
    );
    const approvalRate = stats.opportunities.total
      ? (
          (stats.opportunities.approved / stats.opportunities.total) *
          100
        ).toFixed(1)
      : '0.0';
      ...
      <section className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {[...Array(6)].map((_, index) => (
            <div
              key={index}
              className="neon-border h-32 animate-pulse rounded-lg bg-card/65"
            />
          ))}
        </div>
      </section>
  ...
  const pendingRate = stats.opportunities.total
    ? Math.round(
        (stats.opportunities.pending / stats.opportunities.total) * 100
      )
    : 0;
  ...
  return (
    <section className="space-y-6 text-foreground">
      <header className="space-y-2">
        <h2 className="text-2xl font-bold text-neonMagenta">
          Platform Reports
        </h2>
        <p className="text-sm text-muted">
          A deeper look into user engagement, content activity, and opportunity
          pipeline.
        </p>
      </header>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {reportCards.map((card) => (
          ...
          >
            ...
            <p className="mt-4 text-3xl font-bold text-neonCyan">
              {card.value}
            </p>
      ...
      <section className="space-y-3">
        ...
        <div className="neon-border rounded-lg bg-card/90 p-5">
          <div className="space-y-4 text-sm text-muted-foreground">
            <div>
              <div className="flex justify-between">
                <span>Students</span>
                <span>
                  {stats.users.students.toLocaleString()} ({studentShare}%)
                </span>
              </div>
            ...
            <div>
              <div className="flex justify-between">
                <span>Alumni</span>
                <span>
                  {stats.users.alumni.toLocaleString()} ({alumniShare}%)
                </span>
              </div>
              <div className="mt-2 h-2 rounded bg-white/10">
                <div
        ...
        <div className="neon-border rounded-lg bg-card/90 p-5">
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <div>
              ...
              <p className="mt-2 text-2xl font-bold text-green-200">
                {stats.opportunities.approved}
              </p>
              <p className="text-xs text-muted-foreground">
      ...
      <section className="space-y-3">
        ...
        <div className="neon-border rounded-lg bg-card/90 p-5 text-sm text-muted-foreground">
          <ul className="space-y-2 list-disc pl-5">
            <li>
              Focus on re-engaging inactive users —{' '}
              {stats.users.total - stats.users.active30d} members have not
              interacted in the last 30 days.
            </li>
            <li>
              With {stats.posts.today.toLocaleString()} posts today, content
              moderation should prioritise high-traffic windows.
            </li>
            <li>
              There are {stats.opportunities.pending.toLocaleString()}{' '}
              opportunities awaiting review; keeping approvals timely maintains
