...
const StatCard = ({ label, value, color }: { label: string; value: number; color: string }) => (
  <article className={`night-panel rounded-lg bg-gradient-to-br ${color} p-6 shadow-lg`}>
    <h3 className="text-sm font-semibold text-white/70">{label}</h3>
    <p className="mt-3 text-3xl font-bold text-accent">{value.toLocaleString()}</p>
  </article>
);
  ...
  if (isLoading) {
    return (
      <div className="grid gap-4 p-8 md:grid-cols-2 xl:grid-cols-4">
        {[...Array(8)].map((_, i) => (
          <div key={i} className="night-panel h-24 animate-pulse rounded-lg bg-surface/50" />
        ))}
      </div>
    );
  }
  ...
  return (
    <main className="space-y-8 p-8 text-white">
      <header className="space-y-2">
        <h2 className="text-3xl font-bold text-accentSoft">Admin Dashboard</h2>
        <p className="text-sm text-muted">Real-time platform metrics and health indicators.</p>
      </header>

      {/* Users Section */}
      <section className="space-y-4">
        <h3 className="text-lg font-semibold text-accent">Users Overview</h3>
      ...
      <section className="space-y-4">
        <h3 className="text-lg font-semibold text-accent">Feed Activity</h3>
      ...
      <section className="space-y-4">
        <h3 className="text-lg font-semibold text-accent">Opportunities</h3>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <button
              key={action.label}
              className={`night-panel rounded-lg bg-gradient-to-r ${action.color} px-4 py-3 font-medium text-white transition hover:shadow-lg hover:shadow-accent/50`}
              type="button"
      ...
      <section className="space-y-4">
        <h3 className="text-lg font-semibold text-accent">Featured Alumni Network</h3>
