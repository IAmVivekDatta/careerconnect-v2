    ...
    <section className="space-y-5">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-white/60">{posts.length} posts loaded â€¢ search filters author or content</p>
        </div>
        <input
      ...
      <div className="grid gap-4">
        ...
        {isLoading || isFetching ? (
          ...
          posts.map((post) => (
            <article key={post._id} className="night-panel rounded-lg bg-surface/80 p-5">
              <header className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  <h3 className="text-sm font-semibold text-accent">{post.author.name}</h3>
